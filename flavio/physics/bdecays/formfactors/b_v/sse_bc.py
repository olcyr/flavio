...
def zs_bc(q2,t0):
    zq2 = z(par['m_B'], par['m_Dst'], q2, t0)
    return np.array([1, zq2, zq2**2, zq2**3, zq2**4])

# pole function P(q^2), see Eq.(26) of 2007.06957 
def Ppole(ff,mres,q2):
    mpole = mres[mresdict[ff]]
    return np.prod([z(par['m_B'], par['m_Dst'],q2,mp**2) for mp in mpole])
...
process_dict = {}
process_dict['Bc->psi'] =  {'B': 'Bc', 'V': 'psi',  'q': 'b->c'}
...
def ffBc( q2, par, n=4):
    ...
    for i in ["A0","A1","A2","V"]:
        a = [ par['Bc->psi' + ' SSEbc ' + i.lower() + '_' + 'a' + str(j)] for j in range(n) ]
        ff[i] = Ppole(i, mres, q2)*np.dot(a, zs(mB, mV, q2, t0=(mB-mV)**2)[:n])
    return ff
